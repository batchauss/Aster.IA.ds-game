...
void Vaisseau::move(GLfloat x, GLfloat y, GLfloat z){
    ...
    /*  Si le vaisseau dépasse les limites, il réaparrait de l'autre côté */
...
void Vaisseau::moveForward(){
      GLfloat calculRotationTranslatex = -0.5 * sin(getAngle() * 3.14 / 180);
      GLfloat calculRotationTranslatez =  -0.5 * cos(getAngle() * 3.14 / 180);
