...
void ennemiTouche(){
 for (unsigned int i = 0; i< vaisseau->tirs.size();++i){      
    if(abs(ennemi->posx() - vaisseau->tirs.at(i)->posX()) < ennemi->getLargeur() +vaisseau->tirs.at(i)->posX())
    {
      
      if(abs(ennemi->posy() - vaisseau->posy()) < ennemi->getHauteur() + vaisseau->tirs.at(i)->posY())
      {
          
          if(abs(ennemi->posz() - vaisseau->posz()) < ennemi->getLongueur() + vaisseau->tirs.at(i)->posZ())
          {
